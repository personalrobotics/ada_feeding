...
static const std::vector<std::string> FOOD_NAMES = {
    "apple",         "banana",  "bell_pepper", "broccoli",
    "cantaloupe",    "carrot",  "cauliflower", "celery",
    "cherry_tomato", "grape",   "honeydew",    "kiwi",
    "strawberry",    "lettuce", "spinach",     "kale"};

static const std::vector<std::string> ACTIONS = {"calibrate", "pickupfork",
                                                 "putdownfork"};
...
std::pair<Eigen::VectorXd, Eigen::VectorXd> setPositionLimits(
    ...
    const std::vector<std::size_t> &indices = std::vector<std::size_t>{0, 3, 4,
                                                                       5});
