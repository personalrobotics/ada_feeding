...
//// Planning Nodes
enum PlanToNodeType { kOFFSET, kCONFIG, kTSR, kTWIST };
...
public:
  ...
  BT::NodeStatus onStart() override {
    ...
    switch (T) {
    case kTWIST:
      success = planToTwist(constraint);
      break;
    case kOFFSET:
...
private:
  ...
  bool planToTwist(aikido::constraint::TestablePtr constraint) {
    ...
    mFuture = std::async(
        ...
        [this](Eigen::Vector3d off, Eigen::Vector3d rot,
          ...
          return mAda->getArm()->planToTwist(
...
static void registerNodes(BT::BehaviorTreeFactory &factory,
  ...
  // Planning Functions
  factory.registerNodeType<PlanToNode<kTWIST>>("AdaPlanToTwist", &robot);
