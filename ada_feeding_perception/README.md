...
1. Run the perception nodes: `ros2 launch ada_feeding_perception ada_feeding_perception.launch.py`
1. Launch the motion nodes:
   ...
   1. Real nodes: `ros2 launch ada_feeding ada_feeding_launch.xml`
1. Launch the RealSense node:
   ...
   1. Real nodes:
      ...
      1. On `nano`:
         ...
         1. `ros2 launch realsense2_camera rs_launch.py rgb_camera.profile:='640,480,30' depth_module.profile:='640,480,30' align_depth.enable:='true' initial_reset:='true'`. See here for [a complete list of params](https://github.com/IntelRealSense/realsense-ros/blob/ros2-development/realsense2_camera/launch/rs_launch.py). **Note that `nano` must be connected to the same WiFi network as the computer running any ros2 nodes that subscribe to it.**
      1. (To visualize the camera stream, run `ros2 run rviz2 rviz2`)
      1. **NOTE**: For the compressed depth stream, we found that `png_level` of 1 works best, to get compression while not slowing down FPS too much. Due to a quirk of our eye-in-hand setup, we hardcode this parameter directly in the compressed depth transport; thus, the code does not automatically set that parameter. Be sure to set the parameter yourself!
...
1. **Gather the dataset**: Because this node uses the eye-in-hand camera, it is sensitive to the relative pose between the camera and the fork. If you are using PRL's robot, [the dataset collected in early 2024](https://drive.google.com/drive/folders/1hNciBOmuHKd67Pw6oAvj_iN_rY1M8ZV0?usp=drive_link) may be sufficient. Otherwise, you should collect your own dataset:
   ...
   1. We recorded three types of bags: (a) bags where the robot was going through the motions of feeding without food on the fork and without the fork nearing a person or plate; (b) the same as above but with food on the fork; and (c) bags where the robot was acquiring and feeding a bite to someone. We used the first two types of bags for training, and the third type of bag for evaluation.
   1. All ROS2 bags should be in the same directory, with a file `bags_metadata.csv` at the top-level of that directory.
   1. `bags_metadata.csv` contains the following columns: `rosbag_name` (str), `time_from_start` (float), `food_on_fork` (0/1), `arm_moving` (0/1). The file only needs rows for timestamps when one or both of the latter columns change; for intermediate timestamps, it is assumed that they stay the same.
   1. To generate `bags_metadata.csv`, we recommend launching RVIZ, adding your depth and/or RGB image topic, and playing back the bag. e.g.,
      1. `ros2 run rviz2 rviz2 --ros-args -p use_sim_time:=true`
      1. `ros2 bag play 2024_03_01_two_bites_3 --clock`
      1. Pause and play the rosbag script when food foes on/off the fork, and when the arm starts/stops moving, and populate `bags_metadata.csv` accordingly (elapsed time since bag start should be visible at the bottom of RVIZ2).
1. **Train/test the model on offline data**: We provide a flexible Python script, `food_on_fork_train_test.py`, to train, test, and/or compare one-or-more food-on-fork models. To use it, first ensure you have built and sourced your workspace, and you are in the directory that contains the script (e.g., `cd ~/colcon_ws/src/ada_feeding/ada_feeding_perception/ada_feeding_perception`). To enable flexible use, the script has **many** command-line arguments; we recommend you read their descriptions with `python3 food_on_fork_train_test.py -h`. For reference, we include the command we used to train our model below:
   ```
