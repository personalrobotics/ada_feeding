...
class SwitchArticutoolControllers(BlackboardBehavior):
    ...
    def blackboard_outputs(
        self,
        switch_call_succeeded: Optional[BlackboardKey] = None, # -> bool (Did call complete?)
        switch_response_ok: Optional[BlackboardKey] = None, # -> bool (Payload 'ok' field)
    ) -> None:
        """
        Blackboard Outputs
    ...
    def update(self) -> Status:
        ...
        if self.service_future is None:
            ...
            try:
                ...
                if not isinstance(activate_list, list) or not all(isinstance(s, str) for s in activate_list):
                    self.logger.error(f"[{self.name}] Input 'controllers_to_activate' must be a List[str].")
                    return Status.FAILURE
                if not isinstance(deactivate_list, list) or not all(isinstance(s, str) for s in deactivate_list):
                     self.logger.error(f"[{self.name}] Input 'controllers_to_deactivate' must be a List[str].")
                     return Status.FAILURE
                if not isinstance(strict_level, int):
                     self.logger.error(f"[{self.name}] Input 'strictness' must be an integer.")
                     return Status.FAILURE
                if strict_level not in [SwitchController.Request.BEST_EFFORT, SwitchController.Request.STRICT]:
                     self.logger.warning(f"[{self.name}] Invalid strictness value {strict_level}, using BEST_EFFORT.")
                ...
                req.timeout = Duration(seconds=5.0).to_msg() # Server-side timeout within CM

                # Call service async
                self.logger.info(f"[{self.name}] Calling switch_controller: "
        ...
        # --- State 2: Waiting for Response ---
            ...
            self.logger.debug(f"[{self.name}] Waiting for switch_controller response...")
        ...
        # --- State 3: Processing Response ---
            ...
            response = None
            call_succeeded = False # Did the call itself complete without client-side error?
            ...
            try:
                ...
                if response is not None:
                    ...
                    self.logger.info(f"[{self.name}] switch_controller response: ok={response_ok}")
                else:
    ...
    def terminate(self, new_status: Status) -> None:
        ...
        if self.service_future is not None and not self.service_future.done():
             self.logger.warning(f"[{self.name}] Terminated while waiting for switch_controller response.")
