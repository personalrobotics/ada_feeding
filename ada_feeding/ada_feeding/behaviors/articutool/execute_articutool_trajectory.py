...
class ActionExecutionStatus(Enum):
    ...
    EXECUTING = "EXECUTING"
    WAITING_FOR_RESULT = "WAITING_FOR_RESULT" # If execution status isn't monitored closely
...
class ExecuteArticutoolTrajectory(BlackboardBehavior):
    ...
    # --- Constants ---
    DEFAULT_ACTION_SERVER = "/articutool/joint_trajectory_controller/follow_joint_trajectory"
    ...
    def blackboard_inputs(
        ...
        controllers_to_activate: Union[BlackboardKey, List[str]] = DEFAULT_CONTROLLER_TO_ACTIVATE,
        controllers_to_deactivate: Union[BlackboardKey, List[str]] = DEFAULT_CONTROLLERS_TO_DEACTIVATE,
    ...
    def blackboard_outputs(
        ...
        action_result_code: Optional[BlackboardKey] = None, # -> int (FollowJointTrajectory.Result.error_code)
        action_status: Optional[BlackboardKey] = None, # -> str (from ActionExecutionStatus enum)
    ) -> None:
        """
        Blackboard Outputs
        ...
        try:
            try:
                ...
                if not isinstance(action_server_name, str) or not action_server_name:
                     self.logger.warning(f"[{self.name}] Blackboard 'action_server_name' invalid, using default.")
                     action_server_name = self.DEFAULT_ACTION_SERVER
            except KeyError:
                self.logger.info(f"[{self.name}] Input 'action_server_name' not found, using default: {self.DEFAULT_ACTION_SERVER}")
                action_server_name = self.DEFAULT_ACTION_SERVER
            ...
            if not self._action_client.wait_for_server(timeout_sec=timeout_sec):
                 self.logger.error(f"[{self.name}] Action server '{action_server_name}' not available after {timeout_sec}s during setup.")
            ...
            self.logger.info(f"[{self.name}] Setup complete. Action client created for '{action_server_name}'.")
    ...
    def initialise(self) -> None:
        ...
        self.blackboard_set("action_result_code", self._result_code) # Set to None or initial value
    ...
    def update(self) -> Status:
        ...
        if self._action_client is None or self.controller_switcher is None:
            self.logger.error(f"[{self.name}] Action client or controller switcher not available. Setup failed?")
        ...
        if self._current_status == ActionExecutionStatus.IDLE:
            try:
                ...
                controllers_to_activate: List[str] = self.blackboard_get("controllers_to_activate")
                controllers_to_deactivate: List[str] = self.blackboard_get("controllers_to_deactivate")

                # Validate inputs
                if not isinstance(trajectory, JointTrajectory):
                    self.logger.error(f"[{self.name}] Input 'trajectory' is not a JointTrajectory message.")
                    return Status.FAILURE
                if not trajectory.points:
                    self.logger.warning(f"[{self.name}] Input 'trajectory' has no points. Succeeding trivially.")
                    self._current_status = ActionExecutionStatus.SUCCEEDED
                    self.blackboard_set("action_status", self._current_status.value)
                    self.blackboard_set("action_result_code", FollowJointTrajectory.Result.SUCCESSFUL)
                    return Status.SUCCESS
                if not isinstance(controllers_to_activate, list) or not isinstance(controllers_to_deactivate, list):
                ...
                self.blackboard_set("action_status", self._current_status.value)
                self.logger.info(f"[{self.name}] Requesting controller switch: start={controllers_to_activate}, stop={controllers_to_deactivate}")
                try:
                    ...
                    self.logger.error(f"[{self.name}] Error during controller switch: {sw_e}")
                ...
                self.logger.info(f"[{self.name}] Sending trajectory goal to action server...")
        ...
        if self._current_status == ActionExecutionStatus.SENDING_GOAL:
            if self._send_goal_future is None: return self._handle_invalid_state("SENDING_GOAL future None") # Should not happen

            if self._send_goal_future.done():
                 try:
                     goal_handle: ClientGoalHandle = self._send_goal_future.result()
                 except Exception as e:
                     self.logger.error(f"[{self.name}] Exception getting goal handle from future: {e}")
                     return self._handle_failure("Exception getting goal handle")

                 self._send_goal_future = None # Clear future

                 if not goal_handle.accepted:
                     self.logger.warning(f"[{self.name}] Goal rejected by action server.")
                     self.blackboard_set("action_goal_accepted", False)
                     return self._handle_failure("Goal rejected", ActionExecutionStatus.GOAL_REJECTED)
                 else:
                    ...
                    try:
                        ...
                        self.logger.warning(f"[{self.name}] Could not format goal ID UUID for logging: {log_e}")
                        uuid_hex = "[Error Formatting UUID]"

                    self.logger.info(f"[{self.name}] Goal accepted by action server (ID: {uuid_hex}).")
                    self._goal_handle = goal_handle
                    self.blackboard_set("action_goal_accepted", True)
                    self._get_result_future = self._goal_handle.get_result_async()
                    self._current_status = ActionExecutionStatus.EXECUTING # Or WAITING_FOR_RESULT if preferred
                    self.blackboard_set("action_status", self._current_status.value)
                    return Status.RUNNING
            else:
                # Future not done yet
        ...
        if self._current_status == ActionExecutionStatus.EXECUTING or self._current_status == ActionExecutionStatus.WAITING_FOR_RESULT :
             if self._get_result_future is None or self._goal_handle is None:
                 return self._handle_invalid_state("EXECUTING/WAITING future or handle None")

             # Check goal handle status
             status = self._goal_handle.status
             if status == GoalStatus.STATUS_EXECUTING:
                  self._current_status = ActionExecutionStatus.EXECUTING # Ensure state is correct
                  ...
                  self._current_status = ActionExecutionStatus.WAITING_FOR_RESULT # Move to check result
             ...
             if self._get_result_future.done():
                 try:
                     ...
                     self.logger.info(f"[{self.name}] Action result received: Code={self._result_code}, Msg='{result_string}'")
                     ...
                     elif status == GoalStatus.STATUS_CANCELED: # Check status again if needed
                          return self._handle_failure("Goal Canceled", ActionExecutionStatus.GOAL_CANCELLED)
                     else: # Any other error code implies failure
                         return self._handle_failure(f"Action Failed: Code={self._result_code}, Msg='{result_string}'", ActionExecutionStatus.ABORTED if status == GoalStatus.STATUS_ABORTED else ActionExecutionStatus.FAILED)

                 except Exception as e:
                     self.logger.error(f"[{self.name}] Exception getting result from future: {e}")
        ...
        if self._current_status == ActionExecutionStatus.SUCCEEDED: return Status.SUCCESS
        if self._current_status in [ActionExecutionStatus.FAILED, ActionExecutionStatus.GOAL_REJECTED, ActionExecutionStatus.GOAL_CANCELLED, ActionExecutionStatus.ABORTED]: return Status.FAILURE
    ...
    def terminate(self, new_status: Status) -> None:
        ...
        if new_status == Status.INVALID and self._goal_handle is not None:
            ...
            if status == GoalStatus.STATUS_ACCEPTED or status == GoalStatus.STATUS_EXECUTING:
                 self.logger.warning(f"[{self.name}] Behavior terminated externally while action goal was active (Status: {GoalStatus(status).name}). Requesting cancellation.")
    ...
    def _handle_failure(self, reason: str, final_status: ActionExecutionStatus = ActionExecutionStatus.FAILED) -> Status:
        ...
        if self._result_code is not None and self._result_code != FollowJointTrajectory.Result.SUCCESSFUL:
             self.blackboard_set("action_result_code", self._result_code)
