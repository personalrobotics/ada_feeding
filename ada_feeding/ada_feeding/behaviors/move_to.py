...
# -*- coding: utf-8 -*-
"""
This module defines the MoveTo abstract behavior, which contains the core
logic to move the robot arm using pymoveit2. All behaviors that move the
robot arm, including MoveToConfiguration and MoveToPose, should subclass
MoveTo. Child classes merely have to implement the `initialise_planning`
method, and everything else should fall into place.
"""
# Standard imports
...
class MoveTo(py_trees.behaviour.Behaviour, ABC):
    """
    An abstract class for moving the robot arm. This class creates the MoveIt
    interface, handles the tree tick, generates feedback, and handles termination.
    The only piece of logic that the child class has to implement is the specific
    asynchronous planning call.
    """
    ...
    def __init__(
        ...
        planning_retry_rate_hz: float = 2.0,
        planning_num_retries: int = 3,
    ):
        """
        ...
        terminate_rate_hz: How often to check whether a terminate request has been
            processed.
        planning_retry_rate_hz: How often to retry planning if the planning call
            fails (i.e., the planning service is not available).
        planning_num_retries: How many times to retry planning if the planning call
            fails (i.e., the planning service is not available).
        """
        ...
        self.planning_retry_rate_hz = planning_retry_rate_hz
        self.planning_num_retries = planning_num_retries
    ...
    def plan_async(self) -> Future:
        """
        Launches an asynchronous planning call to MoveIt, and returns
        the Future for that asynchronous call. Note that this method
        intentionally has no arguments, and instead relies on the
        blackboard to store the necessary parameters.
        """
        raise NotImplementedError("plan_async not implemented")
    ...
    def update(self) -> py_trees.common.Status:
        ...
        if self.blackboard.is_planning:  # Is planning
            ...
            if self.planning_future is None:
                ...
                self.planning_future = self.plan_async()
