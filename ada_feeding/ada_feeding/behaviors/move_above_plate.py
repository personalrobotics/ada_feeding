...
# -*- coding: utf-8 -*-
"""
This module defines a behavior for moving above plate. It spins up a
process with MoveIt2's MoveGroup action, and then
sends and receives commands from that. That process sleeps for a
...
def _move_group(
    ...
    """
    This function is designed to call to the MoveGroup action:
    ...
    `plan_time_s` seconds, and moves for `motion_time_s` seconds.

    Parameters
    ----------
    plan_time_s: How many seconds this node should spend in planning.
    motion_time_s: How many seconds this node should spend in motion.
...
class MoveAbovePlate(py_trees.behaviour.Behaviour):

    """
    A behavior for moving above plate. This behavior is designed to call
    MoveIt2's `MoveGroup` action.
    """

    def __init__(
        self,
        name: str,
        plan_time_s: float = 2.5,
        motion_time_s: float = 7.5,
        preempt_timeout_s: float = 10.0,
    ):
        """
        A behavior for moving above plate.

        This behavior will sleep for `plan_time_s` sec, then sleep for
        `motion_time_s` sec, and then succeed.
        ...
        preempt_timeout_s: How long after a preempt is requested to wait for a
            response from the MoveGroup action.
        """
        ...
        # Register read/write access for keys on the blackboard. 
    ...
    def setup(self, **kwargs: int) -> None:
        """
        Start the MoveGroup action server.

        Note that in the actual scenario, this action would already be running.
        """
        self.logger.info("%s [MoveAbovePlate::setup()]" % self.name)
    ...
    def initialise(self) -> None:
        """
        Send a new goal to the MoveGroup action server and reset the
        blackboard.
        """
        self.logger.info("%s [MoveAbovePlate::initialise()]" % self.name)
        ...
        # Send a new goal to the MoveGroup action server
        self.parent_connection.send([ROSACTION_NEW_GOAL])

    def update(self) -> py_trees.common.Status:
        """
        Check if the MoveGroup action server has finished.
        """
        self.logger.info("%s [MoveAbovePlate::update()]" % self.name)

        # Check if the process has died
        if not self.move_group.is_alive():
            self.logger.error(
                "%s [MoveAbovePlate::update()] MoveGroup process died!" % self.name
            )
            return py_trees.common.Status.FAILURE

        # Check the status of the MoveGroup action server
        if self.parent_connection.poll():
            ...
            if response == MOVEGROUP_STATE_PLANNING:
                ...
                if prev_response != response:  # If the robot just started moving
                    ...
                    #       to the goal.
                    self.blackboard.motion_initial_distance = self.motion_time_s
                self.blackboard.motion_time = time.time() - self.start_time
    ...
    def terminate(self, new_status: py_trees.common.Status) -> None:
        ...
        This will cancel any active goal and wait for the MoveGroup action
        server to complete the preemption.
        """
        self.logger.info(
            "%s [MoveAbovePlate::terminate()][%s->%s]"
        ...
        if self.move_group.is_alive():
            ...
            while time.time() - preempt_requested_time < self.preempt_timeout_s:
                if self.parent_connection.poll():
                    ...
                    if response == ROSACTION_GOAL_PREEMPTED:
                        break
        self.logger.info("%s [MoveAbovePlate::terminate()] completed" % self.name)

    def shutdown(self) -> None:
        """
        Shutdown infrastructure created in setup().

        In this case, terminate the MoveGroup action server.
        """
        self.logger.info("%s [MoveAbovePlate::shutdown()]" % self.name)
        # Terminate the MoveGroup action server
